...
async def start_session(req: SessionStartRequest):
    ...
    intake = _dump(req.intake) if req.intake else None
    sid = database.create_session(
        req.patient_hash, cv=cv, doctor_notes=req.doctor_notes,
        worker_id=req.worker_id, facility=req.facility, intake=intake,
    )
...
@app.post("/api/sessions/exercise")
async def set_exercise(payload: dict):
    """Label subsequent telemetry frames with the current movement routine (item 7).
    body: { session_id?, exercise: "flex_ext"|"sit_to_stand"|"single_leg"|null }"""
    if payload.get("session_id"):
        daemon.set_active_session(payload["session_id"])
    daemon.set_active_exercise(payload.get("exercise") or None)
    return {"ok": True, "exercise": daemon._exercise}


@app.get("/api/sessions", response_model=list[SessionSummary])
async def list_sessions(limit: int = 100):
    return [SessionSummary(**r) for r in database.recent_sessions(limit)]
...
async def stop_session(req: SessionStopRequest):
    ...
    daemon.set_active_exercise(None)
...
async def session_report(session_id: str):
    ...
    if session.get("severity_grade") is not None:
        ...
        ml = MLPrediction(
            ...
            risk_index=session.get("risk_index") or 0,
    ...
    return SessionReport(
        ...
        telemetry=telemetry,
        ml=ml,
        ml_severity_grade=session.get("severity_grade"),
...
async def system_status():
    return SystemStatus(
        ...
        model=ml_service.artifacts_status(),
    )
